...
// NOTE: the original project referenced several translation files and an
// external `quran_text_normal.dart` from the `quran` package. In this
// workspace those files are not present (or the package path doesn't expose
// them). To keep functionality and avoid analyzer errors we provide small
// fallbacks here. If you later add the translation files, you can remove
// the fallbacks and restore the imports.
...
import 'package:dartarabic/dartarabic.dart' as ar;

// Fallbacks for missing translation files. These are empty lists so calls
// that iterate translations won't crash the analyzer. Replace with real
// translation data files when available.
const List<Map<String, dynamic>> enSaheeh = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> enClearQuran = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> faHusseinDari = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> itPiccardo = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> nlSiregar = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> portuguese = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> trSaheeh = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> mlAbdulHameed = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> frHamidullah = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> ruKuliev = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> urdu = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> bengali = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> chinese = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> indonesian = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> spanish = <Map<String, dynamic>>[];
const List<Map<String, dynamic>> swedish = <Map<String, dynamic>>[];

// Some earlier code expects `quranTextNormal`. Provide a simple alias to the
// included `quranText` so searches still work (may be slower or partial).
final List<Map<String, dynamic>> quranTextNormal = quranText;
...
String getVerseQCF(
  ...
  // return qcf + (verseEndSymbol ? getVerseEndSymbol(verseNumber) : "");
  return qcf;
}
...
Map<String, dynamic> searchNew(String words) {
  List<Map<String, int>> result = [];
  words = words.replaceAll(' ', '');
  result = quranTextNormal
      .where(
        (surahMap) => surahMap['content']
            .toString()
            .replaceAll(' ', '')
            .normalizeAlef()
            .contains(words),
      ...
      .map(
        (result) => {
          "surah_number": result["surah_number"] as int,
          "verse_number": result["verse_number"] as int,
        },
      )
      .toList();
...
/// returns : List<Map> [{name : "surah_name", number: (surah numbers)}]
List<Map<String, dynamic>> searchSurahs(String name) {
  var result = surah
      .where((item) => item['arabic'].toString().normalizeAlef().contains(name))
      .take(5)
      .map((result) => {'name': result['arabic'], 'number': result['id']});
